...
from .storage import append_entry, load_encrypted_entries, save_all_entries, get_or_create_salt
...
class Crypte:
    def __init__(self, root):
        ...
        self.salt = get_or_create_salt(self.file_path)
    ...
    def ask_master_password(self):
        ...
        if pwd:
            self.key = Fernet(derive_key(pwd, self.salt))
    ...
    def setup_ui(self):
        ...
        search_entry = tk.Entry(search_wrap, textvariable=self.search_var, width=20, bg=BG_INPUT, fg=ACCENT, insertbackground=ACCENT, relief="flat", font=("Segoe UI", 18, "bold"))
    ...
    def copy_selected_field(self, field, label):
        ...
        value = self.passwords_data[idx].get(field, "")
    ...
    def save_all_passwords(self):
        save_all_entries(self.file_path, self.key, self.passwords_data, self.salt)
    ...
    def copy_to_clipboard(self, value, label):
        ...
        messagebox.showinfo("Copie", f"{label} copié (effacement auto dans 30s).")
    ...
    def w_add(self):
        ...
        e_site = create_field("Site")
        e_user = create_field("Utilisateur")
        e_pwd = create_field("Mot de passe", show="•")

        # Logique du petit œil
        def toggle_pwd():
            if e_pwd.cget("show") == "•":
        ...
        btn_eye = ttk.Button(e_pwd.master, text="👁", style="Tiny.TButton", command=toggle_pwd, width=3)
        btn_eye.pack(side="right", padx=(8, 0))
        ...
        def save():
            ...
            append_entry(self.file_path, self.key, entry, self.salt)
            self.load_passwords()
            win.destroy()
    ...
    def open_edit_popup(self, event=None):
        ...
        idx = self.get_selected_index(False)
        if idx is None: return
        ...
        def create_field(label, value, show=""):
            tk.Label(win, text=label, bg=BG_CARD, fg=TEXT_SEC).pack(anchor="w", padx=30, pady=(10, 4))
            row = tk.Frame(win, bg=BG_CARD)
            row.pack(padx=30, fill="x")
            e = ttk.Entry(row, style="Rounded.TEntry", show=show)
        ...
        e_site = create_field("Site", data.get("site", ""))
        e_user = create_field("Utilisateur", data.get("user", ""))
        e_pwd = create_field("Mot de passe", data.get("password", ""), show="•")
        ...
        tk.Label(win, text="Note", bg=BG_CARD, fg=TEXT_SEC).pack(anchor="w", padx=30, pady=(10, 4))
        t_note = tk.Text(win, bg=BG_INPUT, fg=ACCENT, height=5, bd=0, padx=8, pady=8)
        t_note.pack(padx=30, fill="x")
